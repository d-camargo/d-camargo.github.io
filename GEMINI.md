...
> **This file (`GEMINI.md`) is the single source of truth for this repo's rules.**
> `CLAUDE.md` and `AGENTS.md` are relative symlinks pointing here. **Always edit
> `GEMINI.md`** — `agy` (Antigravity) does not follow symlinks and only reads a
> real file, and Claude Code's Edit tool refuses to write through a symlink.
> Measured 2026-08-10.
