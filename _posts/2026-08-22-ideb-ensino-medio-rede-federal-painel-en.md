...
The way out is to restrict the comparison to schools with published data in both years being compared, a balanced panel. That discards most of the 596 federal schools ever listed (leaving 70 in the 2019-to-2025 cut, nationally), but it guarantees the measured change comes from the same school at both points, not from schools entering and leaving the sample.
...
$$flow\_effect = (P_1 - P_0) \times \frac{N_0 + N_1}{2}$$

$$learning\_effect = (N_1 - N_0) \times \frac{P_0 + P_1}{2}$$

The first isolates how much of the product changed because the pass rate changed, holding the score at the two years' average; the second isolates how much changed because the score changed, holding the pass rate at the average. The two together close exactly against the change in $P \times N$, with no residual.
...
|---|---|---|---|---|---|
| Federal (70 schools) | 2019 | 5.63 | 0.9203 | 92.03% | 6.10 |
| Federal (70 schools) | 2025 | 5.42 | 0.9362 | 93.62% | 5.79 |
| State (10,713 schools) | 2019 | 4.17 | 0.8930 | 89.30% | 4.65 |
| State (10,713 schools) | 2025 | 4.53 | 0.9622 | 96.22% | 4.70 |
...
The reading is the same in both networks, and it is the point of this post: where the pass rate still had room to rise, the index rose with the Saeb score essentially flat (in the state network, close to 90% of the gain comes from flow). Where the pass rate was already near its ceiling, as in the federal network, the room ran out, and the falling score showed up almost entirely in the index: the negative learning effect (-0.2959) is larger in absolute value than the product's own drop, because flow still pulled a little upward and was not enough to offset it.
...
**Selection bias.** The panel uses 70 of the 596 federal schools ever listed in any edition. These are the schools that managed to keep reporting in both years compared, which is already, by itself, a non-random trait.
